...
test('server console page uses the admin-style title format and simplified websocket console layout', () => {
    ...
    assert.match(consolePageContents, /ResponsiveContainer/);
    assert.match(consolePageContents, /AreaChart/);
    assert.match(consolePageContents, /title="Memory"/);
    assert.match(consolePageContents, /title="CPU"/);
    assert.match(consolePageContents, /title="Disk"/);
    assert.match(consolePageContents, /Live usage/);
    assert.match(consolePageContents, /updates every 2s/);
    ...
    assert.match(consolePageContents, /onClick=\{\(\) => void sendPowerSignal\("restart"\)\}/);
    assert.match(consolePageContents, /onClick=\{\(\) => void sendPowerSignal\("stop"\)\}/);
    assert.match(consolePageContents, /onClick=\{\(\) => setConfirmingSignal\("kill"\)\}/);
    assert.doesNotMatch(consolePageContents, /setConfirmingSignal\("restart"\)/);
    assert.doesNotMatch(consolePageContents, /setConfirmingSignal\("stop"\)/);
    assert.match(consolePageContents, /Kill server/);
    assert.doesNotMatch(consolePageContents, /Restart server/);
    assert.doesNotMatch(consolePageContents, /Stop server/);
